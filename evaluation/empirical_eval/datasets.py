...
class Dataset:
    ...
    def get_train_valid_splits(self, valid_portion, n_splits, shuffle=False, random_state=None):
        ...
        for i in reversed(range(n_splits)):
            ...
            idx_train, idx_valid = (
                np.concatenate([idx[:idx_start], idx[idx_end:]]),
...
class EuroStoxx50(Dataset):
    ...
    def get_train_valid_splits(
        self, valid_portion, n_splits, shift_size=100, shuffle=False, random_state=None
    ):
        ...
        for i in reversed(range(n_splits)):
            ...
            idx_train, idx_valid = (
                idx[idx_train_start:idx_valid_start],
                idx[idx_valid_start:idx_valid_end],
            )
...
class NCYTaxiDropoffPredict(Dataset):
    ...
    feature_columns = [
        "pickup_loc_lat",
        "pickup_loc_lon",
        "pickup_time_day_of_week_sin",
        "pickup_time_day_of_week_cos",
        "pickup_time_of_day_sin",
        "pickup_time_of_day_cos",
    ]
    ...
    def _process_df(self, df):  # does some data cleaning
        ...
        times = np.array(
            [_process_time(d_pickup, d_dropoff) for (d_pickup, d_dropoff) in data[:, 1:3]]
        )
        pickup_time = times[:, :2]
        ...
        df_processed = pd.DataFrame(
            {
                ...
                "dropoff_time_of_day": dropoff_time_of_day,
                "duration": duration,
            }
        )
    ...
    def __str__(self):
        return "%s (n_samples = %i, ndim_x = %i, ndim_y = %i)" % (
            str(self.__class__.__name__),
            self.n_samples,
            self.ndim_x,
            self.ndim_y,
...
if __name__ == "__main__":
    for dataset_class in [EuroStoxx50, NCYTaxiDropoffPredict] + UCI_DATASETS:
        ...
        n_samples = Y.shape[0]
        print(
            "%s: n_samples = %i, ndim_x = %i, ndim_y = %i"
            % (str(dataset.__class__.__name__), n_samples, dataset.ndim_x, dataset.ndim_y)
        )
